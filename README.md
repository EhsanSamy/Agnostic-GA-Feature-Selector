...
1. Clone the repository:
   ```
   git clone https://github.com/EhsanSamy/Agnostic-GA-Feature-Selector.git
   ```
...
3. Prepare datasets: Place CSV files in `data/` (e.g., `diabetes_raw_data.csv`). The last column should be the target variable.
...
- `notebooks/02_baseline_experiments.ipynb`: Training baseline model on the dataset.
- `notebooks/03_ga_optimization.ipynb`: Analyze experiment outputs, visualize comparisons.
...
- Data: `HANDLE_MISSING` (mean/KNN/drop), `NORMALIZE` (True/False).
...
FeatureSelectionGA/
├── data/
│   ├── diabetes_raw_data.csv #original datasets (.csv)
├── src/
│   ├── __init__.py
...
├── experiments/
│   └── compare_operators.py  # Runs 27 configs (3×3×3) × 2, saves results
├── gui/
...
│   └── 02_baseline_experiments.ipynb # Training baseline model 
│   └── 03_ga_optimization.ipynb # Test single experiment 
├── results/
